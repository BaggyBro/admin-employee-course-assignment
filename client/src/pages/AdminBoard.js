...
const AdminBoard = () => {
  ...
  const [TMvisible, setTMVisible] = useState(true)
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      try {
        ...
        if (data.user._id !== userId || !data.user.admin) {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-white shadow-lg rounded-lg p-8 w-30 mt-5 border-black">
        <button onClick={() => {setMTVisible(false); setTMVisible(true)}} className="bg-altresblue text-white p-2 rounded-2xl w-70">Team Members</button>
        <button onClick={() => {setTMVisible(false); setMTVisible(true)}} className="bg-altresblue text-white p-2 rounded-2xl">My Training</button>
        <div>
          {TMvisible && <TeamMembers/>}
          {MTvisible && <MyTraining/>}
        </div>
      </div>
    </div>
  );
